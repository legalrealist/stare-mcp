...
// Opinion types that count as "lead" for auto-selection
      ...
      if (!clCursor && isCitation(query)) {
        ...
        for (const c of citResult.clusters) {
          ...
          if (opResult.error) {
            ...
            if (opResult.skipped_opinions) {
              entry.skipped_opinions = opResult.skipped_opinions;
            }
          }
    ...
    if (!resolvedOpinionId && cluster_id) {
      try {
        ...
        if (opList.opinions.length === 0) {
          return jsonResponse(
            errorEnvelope(
              makeError("not_found", `Cluster ${cluster_id} has no opinions.`),
              { cluster_id }
            )
          );
        }

        // If any opinion metadata fetches failed, the visible set is incomplete.
        // Never auto-select — we might be seeing only the dissent because the
        // lead hit a 429. Return what we have and let the caller decide.
        if (opList.skipped_opinions > 0) {
          const err = makeError(
            "selection_required",
            `Cluster ${cluster_id}: ${opList.skipped_opinions} opinion(s) could not be fetched. Cannot safely auto-select. Specify opinion_id.`
          );
          err.opinions = opList.opinions;
          err.skipped_opinions = opList.skipped_opinions;
          err.partial = true;
          err.cluster_id = cluster_id;
          err.case_name = opList.case_name;
          return jsonResponse(errorEnvelope(err, { cluster_id }));
        }

        // Auto-select if exactly one lead opinion and full visibility
        const leads = opList.opinions.filter((o) => LEAD_TYPES.has(o.type));
        if (leads.length === 1) {
          resolvedOpinionId = leads[0].opinion_id;
        } else if (opList.opinions.length === 1) {
          // Only one opinion total — use it regardless of type
          resolvedOpinionId = opList.opinions[0].opinion_id;
        } else {
          // Multiple substantive opinions — require explicit selection
          // Include the opinion list so the caller can choose
          const err = makeError("selection_required", `Cluster ${cluster_id} has ${opList.opinions.length} opinions. Specify opinion_id.`);
          err.opinions = opList.opinions;
          err.cluster_id = cluster_id;
          err.case_name = opList.case_name;
          return jsonResponse(errorEnvelope(err, { cluster_id }));
