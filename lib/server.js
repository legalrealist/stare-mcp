...
server.tool(
  "fetch_passages",
  "Retrieve paragraph-aligned text from a specific opinion. Pass opinion_id for direct retrieval, or cluster_id to auto-select the lead opinion (returns selection_required if multiple substantive opinions exist). Returns stable fragment IDs (cl:{opinion_id}:p{index}) for citation. No heuristic labels — text is returned as-is.",
  {
    ...
    if (!resolvedOpinionId && cluster_id) {
      try {
        ...
        if (leads.length === 1) {
          ...
          return jsonResponse(
            errorEnvelope(
              makeError("selection_required", `Cluster ${cluster_id} has ${opList.opinions.length} opinions. Specify opinion_id.`),
              { cluster_id }
            )
          );
        }
