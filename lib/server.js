...
if (flag === "--help" || flag === "-h") {
  console.log(`stare-mcp v0.1.0 — Exploratory federal case law search
...
async function handleCitation(query, token) {
  ...
  const shown = allFragments.slice(0, MAX_FRAGMENTS);
  const truncated = allFragments.length - shown.length;
  ...
  lines.push(`\n*${shown.length} of ${allFragments.length} fragments shown.*`);
  if (truncated > 0) {
    lines.push(`*${truncated} additional fragments truncated to manage context size.*`);
  }
...
server.tool(
  "research",
  "Exploratory federal case law search. Searches CourtListener and sorts results by court level (SCOTUS > circuit > district). Pass a legal question to search, or a citation (e.g. '511 U.S. 825') to fetch a specific opinion. Limitations: results depend on keyword relevance ranking and may miss controlling authority; no citator or negative treatment check; section labels are heuristic. Use for finding starting points, not establishing the law.",
  {
  ...
  async ({ query, circuit }) => {
    ...
    if (circuitErr) {
      return {
        content: [{ type: "text", text: `[ERROR] ${circuitErr}` }],
        isError: true,
      };
    }
